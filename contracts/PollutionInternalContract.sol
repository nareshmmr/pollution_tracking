...
contract PollutionInternalContract is PluginClient, Ownable {
  ...
  function showAQI() public view returns(uint256){
    return currentValue;
  }
  ...
  function requestAQIData(address _caller,string memory latitude, string memory longitude)
    public
    returns (bytes32 requestId)
  {
    ...
    req.add("_lat",latitude);
    req.add("_lon",longitude);
 ...
 function testMyFunc(string memory latitude, string memory longitude)
    public
    onlyOwner
    returns (bytes32 requestId)
  {    
